    ...
    if (code>='0' && code<='9') {
        ...
        static const uint16_t map[] =
             {  KEY_HOME, KEY_LEFT, KEY_UP, KEY_RIGHT, KEY_DOWN,
                KEY_PAGEUP, KEY_PAGEDOWN, KEY_POWER, 0 };
        ...
        switch (code) {
            ...
            case 0xFF1B:    scancode = KEY_BACK;        break; //ESC to BACK
            ...
            case 0xFFC5:    scancode = KEY_F8;       break; // F8
            case 0xFFC9:    scancode = KEY_MENU;       break; // F12 to MENU
...
static void query_dev_info()
{
    const char devdir[]="/dev/input";
    char name[256];
    unsigned char key_bitmask[(KEY_MAX + 1) / 8];
    unsigned char abs_bitmask[(ABS_MAX + 1) / 8];
    int fd;
    int i, j;
    int found_kbd = 0, found_ts = 0;

    for (i = 0; i < 32; i++) {
        memset(name, 0, sizeof(name));
        sprintf(name, "%s/event%d", devdir, i);
        fd = open(name, O_RDWR);
        if(fd < 0)
            continue;
        ...
        if (test_bit(ABS_MT_POSITION_X, abs_bitmask)
                ...
                && test_bit(BTN_TOUCH, key_bitmask)) {
            found_ts = 1;
            mt_touch = 1;
            memset(TOUCH_DEVICE, 0, sizeof(TOUCH_DEVICE));
            strcpy(TOUCH_DEVICE, name);
            continue;
        }

        if (!found_ts && test_bit(BTN_TOUCH, key_bitmask)
        ...
        for (j = 0; !found_kbd && j < ((BTN_MISC + 7)/8); j++) {
            if (key_bitmask[j] != 0) {
                found_kbd = 1;
                memset(KBD_DEVICE, 0, sizeof(KBD_DEVICE));
                strcpy(KBD_DEVICE, name);
                break;
            }
        }

        close(fd);
        if (found_kbd && found_ts)
            break;
    }
    if (!found_kbd)
        strcpy(KBD_DEVICE, "/dev/input/event32");
    if (!found_ts)
        strcpy(TOUCH_DEVICE, "/dev/input/event32");
