 ...
 */
void
woal_dump_firmware_info(moal_private * priv)
{
    ...
    do {
        ...
        *dbg_ptr =
        ...
        case DEBUG_ITCM_DONE:
            ...
            moal_get_system_time(phandle, &sec, &usec);
            PRINTM(MMSG, "Start DTCM output %u.%06u, please wait...\n", sec,
                   usec);
            break;
        case DEBUG_DTCM_DONE:
            ...
            moal_get_system_time(phandle, &sec, &usec);
            PRINTM(MMSG, "Start SQRAM output %u.%06u, please wait...\n", sec,
                   usec);
            break;
    ...
    if (IS_ERR(pfileITCM)) {
        ...
        pfileSQRAM = filp_open("/var/file_SQRAM", O_CREAT | O_RDWR, 0644);
    } else {
        PRINTM(MMSG,
               "Now Start to save all the outputs to the /data directory: %u.%06u, please wait...\n",
               sec, usec);
        pfileDTCM = filp_open("/data/log/file_DTCM", O_CREAT | O_RDWR, 0644);
        pfileSQRAM = filp_open("/data/log/file_SQRAM", O_CREAT | O_RDWR, 0644);
    }
    if (IS_ERR(pfileITCM)) {
