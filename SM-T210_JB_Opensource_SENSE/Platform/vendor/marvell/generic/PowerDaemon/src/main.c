...
static void *ppd_socket_handler (void *arg) {
	fd_set readfds;
    int tid, ret, read_fd = *(int *)arg;
    unsigned int plugin_tag = 0;
    int data_len;
	...
	while (1) {
		...
		if(FD_ISSET(read_fd, &readfds)) {
            ...
            if (cnt == sizeof(head)) {
                ...
                cnt=0;
                while(cnt < data_len){
                    cnt += read(read_fd, buf+cnt, data_len-cnt);
                }
...
//                pthread_cleanup_push(pthread_mutex_unlock, (void *)&config->ppd_mutex);
                ...
                LIST_FOREACH_NODE(node, &config->plugins) {
                    ...
                    if(o_plugin->plugin->plugin_tag == plugin_tag) {
...
static unsigned int get_chip_revision(void) {
    FILE *fp;
    char line[256];
	const char *split = ": ";
