...
static int coda_update(void *data) {
	...
	switch(update_info->event) {
		case CODA_START:
			if(!found) {
				clog(LOG_DEBUG, "append the instance %d\n", update_info->tid);

				pNode = node_new(update_info, sizeof(struct coda_info));
				if (pNode == NULL) {
					clog(LOG_ERR, "cannot make enough room for a new instance (%d)\n", update_info->tid);
					return -1;
				}
				list_append(coda_list, pNode);

				mark_keyword_status("coda_info");
				plugin_coda->total_instances++;
				clog(LOG_DEBUG, "coda instances[%d/%d]\n", plugin_coda->update_instances, plugin_coda->total_instances);
			}
			else {
				clog(LOG_ERR, "couldn't append an instance %d, already exist!\n", update_info->tid);
			}
			break;

		case CODA_UPDATE:
			if(found) {
				clog(LOG_DEBUG, "update the instance %d\n", update_info->tid);

				if(ci->event == CODA_START)
					plugin_coda->update_instances++;
				memcpy(ci, update_info, sizeof(struct coda_info));

				mark_keyword_status("coda_info");
				clog(LOG_DEBUG, "coda instances[%d/%d]\n", plugin_coda->update_instances, plugin_coda->total_instances);
			}
			else {
				clog(LOG_ERR, "couldn't update the instance %d, not exist!\n", update_info->tid);
			}
			break;

		case CODA_STOP:
			if(found) {
				clog(LOG_DEBUG, "remove the instance %d\n", update_info->tid);
				...
				clog(LOG_ERR, "couldn't remove the instance %d, not exist!\n", update_info->tid);
			}
			break;
