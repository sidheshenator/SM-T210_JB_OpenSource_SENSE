...
static int convert_to_android_prio(int prio)
{
	/* map the prio of syslog to android level */
	switch (prio) {
		case LOG_EMERG:
		case LOG_ALERT:
		case LOG_CRIT:
...
void ppd_log(int prio, const char *fmt, ...) {
	...
	if (configuration->android_log) {
		__android_log_vprint(convert_to_android_prio(prio),
				    "ppd", fmt, argp);
